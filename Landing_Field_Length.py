...
def calc_landing_FL_W_S(W_S,  M_cruise, sweep, AR, density, alt, Cdo, LNDFL, max_fuel_LND_ratio, range):
    CL_max_LND = Mdd_constraint(W_S, M_cruise, alt, sweep, AR)[1]

    ##Descent
    CL = CL_max_LND / (1.3**2)
    L_D_descent = CL / drag_constant_Cdo(CL, CL_max_LND,AR,  True, 'LND',0, M_cruise, Cdo)[0]
...
def iterate_W_S_LND(W_S,  M_cruise, sweep, AR, alt=0, Cdo=0.015, LNDFL=9000, max_fuel_LND_ratio=0.45, range = 7000, temp_delta=22.6):
    ...
    while np.abs(W_S_new - W_S) > 0.001:
        W_S = W_S_new
        W_S_new = calc_landing_FL_W_S(W_S, M_cruise, sweep, AR, densityHot, alt, Cdo, LNDFL, max_fuel_LND_ratio, range)
