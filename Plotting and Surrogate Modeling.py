...
def file_plot_type_3 (f, xLabel, yLabel, title, deg):   #Plots and creates polyfit for each curve for functions where x and y are log scales
    plt.figure()
    ...
    for i in range(len(data_header)):
        ...
        #poly fit
        coeffs = np.polyfit(np.log10(X), np.log10(Y), deg)  # takes inputs X, Y, degree polynomial
        plt_coeffs = np.poly1d(coeffs)
        Y_fit = 10 **(plt_coeffs(np.log10(X))) #Log scaling adjustment
        plt.plot(X, Y_fit)
        print(data_header[i], coeffs)
...
#file_plot_type_1('Plots/JT9D_SL.csv', 'Mach', 'Thrust (1k lbs)', 'JT9D Sea Level', 4)
